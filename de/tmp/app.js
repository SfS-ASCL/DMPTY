...
.config(function($stateProvider, $urlRouterProvider) {
	...
	.state('form.project', {
        ...
        .state('form.dataDesc', {
            url: '/dataDesc',
            templateUrl: 'form-dataDesc.html'
        })
        ...
        .state('form.ethicsLicence', {
            url: '/ethicsLicence',
            templateUrl: 'form-ethicsLicence.html'
        })

        // storage and backup
        .state('form.storageBackup', {
            url: '/storageBackup',
            templateUrl: 'form-storageBackup.html'
        })
    
        // storage and backup
	.state('form.archiveMgr', {
            url: '/archiveMgr',
            templateUrl: 'form-archiveMgr.html'
        })
    
        // url will be /form/timetable
        .state('form.timetable', {
            url: '/timetable',
            templateUrl: 'form-timetable.html'
        })
        ...
        .state('form.dataSharing', {
            url: '/dataSharing',
            templateUrl: 'form-dataSharing.html'
        })
...
//		console.debug('centres', $scope.centres[c].pk);
	    ...
	    // add ANOTHER OPTION
	    $scope.clarind_centres.push( { name: "Anderes Zentrum (im Dokument vermerken)",
					   ac: 999,
					   tc: 999
					 });
...
//    console.log('fetching CLARIN contact data');
	...
	if (newVal) {
	    ...
	    if (!found) {
		$scope.clarinContacts.push( "Ansprechpartner im Dokument benennen!" );
		$scope.formData.clarinContact = $scope.clarinContacts[0];
	    } else {
  ...
  $scope.dataDelete_open = function($event) {
	...
	var data = document.getElementById("researchDataPlan.html").innerHTML;
	console.debug('plan dynamic to be posted to cgi-bin', data);
	...
	case "Microsoft Word (docx)":
	    ...
	    $http.post('/cgi-bin/gen_docx.cgi', data, {responseType: 'arraybuffer'}
			  ...
			  element.href = url;
			  element.download = 'dataMgtPlan.docx';
			  document.body.appendChild(element);
			  element.click();
			  setTimeout(function(){
			      alert('Der Plan wurde in Microsoft Word (docx) heruntergeladen !');
			      document.body.removeChild(element);
			      (window.URL || window.webkitURL).revokeObjectURL(url); 
			  }, 20);  
			  
		      }).error(function(){
			  alert("Es ist ein Fehler bei der Generierung des Word Dokumentes aufgetreten.");
		      });	    
	    break;
	...
	case "Rich Text Format (rtf)":
	    ...
	    $http.post('/cgi-bin/gen_rtf.cgi', data, {responseType: 'arraybuffer'}
			  ...
			  element.style = "display: none";  
			  element.href = url;
			  element.download = 'dataMgtPlan.rtf';
			  document.body.appendChild(element);
			  element.click();
			  setTimeout(function(){
			      alert('Der Plan wurde in RTF heruntergeladen !');
			      document.body.removeChild(element);
			      (window.URL || window.webkitURL).revokeObjectURL(url); 
			  }, 200);  

		      }).error(function(){
			  alert("Es ist ein Fehler bei der Generierung des RTF Dokumentes aufgetreten.");
		      });	    
	    break;
	case "LaTeX":
	    delete $http.defaults.headers.common['X-Requested-With'];
	    $http.post('/cgi-bin/gen_latex.cgi', data, {responseType: 'arraybuffer'}
			  ...
			  element.style = "display: none";  
			  element.href = url;
			  element.download = 'dataMgtPlan.tex';
			  document.body.appendChild(element);
			  element.click();
			  setTimeout(function(){
			      alert('Der Plan wurde in LaTeX heruntergeladen !');
			      document.body.removeChild(element);
			      (window.URL || window.webkitURL).revokeObjectURL(url); 
			  }, 200);  
			  
		      }).error(function(){
			  alert("Es ist ein Fehler bei der Generierung des LaTeX Dokumentes aufgetreten.");
		      });	    
	    break;	    

	default:
	    alert('Bitte wählen Sie ein Dokumentenformat!');
	}
    }
})
